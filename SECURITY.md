...
read-only mode, which rejects every mutating `/api/` request. Keep the reverse
proxy responsible for TLS, request limiting, access logs, and host-level access
controls.
...
Use a unique production database password, keep backups outside the deployment
host, and test restores. Dependency updates and the production Compose smoke
test run through GitHub configuration in this repository.
