...
class CodeSearchStoreTest {
    ...
    void ignoresQueriesShorterThanTwoCharacters() {
        ...
        assertThat(response.callables()).isEmpty();
    ...
    void searchesEachSupportedCategory() {
        CodeSearchResult result = new CodeSearchResult(
                ...
                "dev.springbootstaticanalysis.indexing",
        ...
        assertThat(response.callables()).containsExactly(result);
    ...
    @Test
    @SuppressWarnings("unchecked")
    void searchesTheCallableNameAndSignatureAsDisplayed() {
        when(jdbc.query(anyString(), anyMap(), any(RowMapper.class)))
                .thenReturn(List.of());

        search.search(UUID.randomUUID(), " search(UUID, ");
        ...
        verify(jdbc, times(3)).query(
                statements.capture(), parameters.capture(), any(RowMapper.class));
        assertThat(statements.getAllValues()).anySatisfy(statement ->
                assertThat(statement).contains(
                        "lower(s.simple_name || COALESCE(s.signature, '')) LIKE :pattern"));
        assertThat(parameters.getAllValues()).allSatisfy(values ->
                assertThat(values.get("pattern")).isEqualTo("%search(uuid,%"));
    }
}
