...
public class CodeSearchStore {
    ...
    public CodeSearchResponse search(UUID repositoryId, String query) {
        ...
        return new CodeSearchResponse(
                ...
                callables(parameters),
                files(parameters));
    }

    private List<CodeSearchResult> endpoints(Map<String, Object> parameters) {
        return jdbc.query("""
                SELECT e.id, e.controller_method_id, e.path AS label,
    ...
    private List<CodeSearchResult> callables(Map<String, Object> parameters) {
        return jdbc.query("""
                SELECT s.id, s.kind,
                ...
                WHERE s.repository_id = :repositoryId
                  ...
                  AND (lower(s.simple_name) LIKE :pattern
                       OR lower(s.simple_name || COALESCE(s.signature, '')) LIKE :pattern
